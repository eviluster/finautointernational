<template>
  <div
    class="modal fade"
    id="kt_modal_edit_traccion"
    ref="editTraccionModalRef"
    tabindex="-1"
    aria-hidden="true"
  >
    <!--begin::Modal dialog-->
    <div class="modal-dialog modal-dialog-centered mw-650px">
      ...
      <div class="modal-content">
        <!--begin::Modal header-->
        <div class="modal-header" id="kt_modal_edit_traccion_header">
          <!--begin::Modal title-->
          <h2 class="fw-bold">Editar Traccion</h2>
          <!--end::Modal title-->

          <!--begin::Close-->
          <div
            id="kt_modal_edit_traccion_close"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const editTraccionModalRef = ref<HTMLElement | null>(null);
    ...
    // Sincronizamos los datos de la prop "traccion" con el formulario
    watchEffect(() => {
      if (props.traccion) {
        formData.value.id = props.traccion.id || "";
        formData.value.nameEs = props.traccion.name?.es || "";
        formData.value.nameEn = props.traccion.name?.en || "";
        formData.value.descriptionEs = props.traccion.description?.es || "";
        formData.value.descriptionEn = props.traccion.description?.en || "";
      }
    });
    ...
    const handleSubmit = async (
      ...
      try {
        ...
        // Llamamos a updateTraccion pasando el ID y el objeto con los datos actualizados
        await traccionStore.updateTraccion(formData.value.id, updatedData);
        await traccionStore.fetchTraccion(); // Refrescamos la lista de traccions

        hideModal(editTraccionModalRef.value);
        resetForm();
      } catch (error) {
