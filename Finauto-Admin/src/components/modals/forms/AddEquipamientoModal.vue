<template>
  <div
    class="modal fade"
    id="kt_modal_add_equipamiento"
    ref="addEquipamientoModalRef"
    tabindex="-1"
    aria-hidden="true"
  >
    <div class="modal-dialog modal-dialog-centered mw-650px miModal">
      <div class="modal-content">
        <div class="modal-header" id="kt_modal_add_provincia_header">
          <h2 class="fw-bold">Añadir un Equipamiento</h2>
          <div
...
interface AddEquipamientoFormFields {
  name: string;
  description: string;
  precioBase: number;
  galery: string[];
...
export default defineComponent({
  ...
  setup() {
    ...
    const { errors } = useForm<AddEquipamientoFormFields>({
      validationSchema: schema,
    });

    const handleSubmit = async (
      values: AddEquipamientoFormFields,
      ...
      try {
        const response =
          await equipamientoStore.addEquipamiento(newEquipamiento);
        await equipamientoStore.fetchEquipamientos();
        Swal.fire({
          text: "Equipamiento agregado correctamente",
          icon: "success",
        ...
        Swal.fire({
          text: "Error al crear el equipamiento",
          icon: "error",
          buttonsStyling: false,
          confirmButtonText: "Ok",
          heightAuto: false,
        ...
        hideModal(addEquipamientoModalRef.value);
        resetForm();
        imageUrls.value = [];
      }
    };
